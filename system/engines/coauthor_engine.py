...
class CoAuthorEngine:
    ...
    def write_next_chapter(self, target_chapter_num: int = 1, pov: str = "Nguyễn Minh An (Ngôi thứ nhất)", custom_draft_prose: str = None) -> dict:
        ...
        # 8. Git Commit Minor với manifest rõ ràng
        self.git_manager.commit_minor(
            f"Cập nhật hoàn chỉnh Chương {target_chapter_num} (Markdown + DOCX)",
            files=[md_file, docx_file]
        )
...
"""
    ...
    def _update_state_post_chapter(self, chapter_num: int, chapter_title: str = ""):
        ...
        # Plot node update
        arc_id = "arc_01" if chapter_num <= 46 else ("arc_02" if chapter_num <= 100 else "arc_03")
        cur.execute("""INSERT OR REPLACE INTO plot_nodes (id, node_type, parent_id, order_index, title, status)
                       VALUES (?, 'CHAPTER', ?, ?, ?, 'CANONIZED')""",
                    (f"ch_{chapter_num:03d}", arc_id, chapter_num, title_clean))
        conn.commit()
        conn.close()

        if chapter_num <= 22:
            from system.fixtures.bootstrap_chapters import apply_bootstrap_chapter
            apply_bootstrap_chapter(self.db_path, chapter_num)
        else:
            conn = sqlite3.connect(self.db_path, timeout=30.0)
            cur = conn.cursor()
